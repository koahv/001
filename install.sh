...
function execfunc00 {
	echo You need to configure partitions prior to install. The script requires one \(225mb\) FAT32 EFI partition at the start of the drive and one partition for the system. Have dev identifiers ready.
	echo
	echo This type of installation does not yet separate the root filesystem and any home directories into different partitions. Troubleshoot fstab first if necessary.
	echo
	qcontinue00
#	lsfs00
	lsfs01
...
function lsfs02 {
	echo Installation Media Selected:; echo
	lsscsi;	echo
	lsblk $fsid00; echo
	lsblk $fsid01
}
